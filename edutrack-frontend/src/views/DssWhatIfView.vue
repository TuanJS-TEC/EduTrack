...
const calculateWhatIf = async () => {
  ...
  try {
    const payload = {
      ...
      diemCuoiKyGiaDinh: parseFloat(form.value.hypotheticalFinal),
      targetTb: parseFloat(form.value.targetAverage)
    }
...
// UI Helpers
const formatClassification = (label) => {
  if (!label) return 'Chưa XL'
  const map = {
    'Gioi': 'Giỏi',
    'Kha': 'Khá',
...
const getGradeColor = (score) => {
  ...
  if (s >= 8.0) return 'text-green-500 dark:text-green-400'
  if (s >= 6.5) return 'text-blue-500 dark:text-blue-400'
  if (s >= 5.0) return 'text-orange-500 dark:text-orange-400'
  return 'text-red-500 dark:text-red-400'
}

const getRequiredScoreColor = (score) => {
  if (score > 10) return 'text-red-500 dark:text-red-400' // Impossible
  if (score >= 8) return 'text-orange-500 dark:text-orange-400' // Hard
  if (score >= 5) return 'text-blue-500 dark:text-blue-400' // Normal
  return 'text-green-500 dark:text-green-400' // Easy
}

const classificationColor = computed(() => {
  const label = result.value.classification
  if (label === 'Giỏi' || label === 'Gioi') return 'bg-green-100 dark:bg-green-500/20 text-green-700 dark:text-green-400'
  if (label === 'Khá' || label === 'Kha') return 'bg-blue-100 dark:bg-blue-500/20 text-blue-700 dark:text-blue-400'
  if (label === 'TrungBinh' || label === 'Trung Bình' || label === 'TB') return 'bg-orange-100 dark:bg-orange-500/20 text-orange-700 dark:text-orange-400'
  if (label === 'Yếu' || label === 'Kém' || label === 'Yeu' || label === 'Kem') return 'bg-red-100 dark:bg-red-500/20 text-red-700 dark:text-red-400'
  return 'bg-gray-100 dark:bg-white/5 text-gray-700 dark:text-gray-300'
})
</script>
          ...
          <div class="bg-white dark:bg-[#111C44] rounded-2xl shadow-sm border border-gray-100/50 dark:border-white/5 p-6 relative overflow-hidden group border-b-4 border-b-[#1E88E5]">
            ...
            <div class="flex items-end gap-3 min-h-[72px]">
              <span class="text-[72px] font-black leading-none tracking-tighter drop-shadow-sm" :class="getRequiredScoreColor(result.requiredFinalScore)">
                {{ result.requiredFinalScore > 10 ? 'N/A' : Math.max(0, result.requiredFinalScore).toFixed(1) }}
              </span>
            ...
            <div class="mt-6 pt-4 border-t border-gray-100 dark:border-white/5 flex items-start gap-2">
              ...
              <CheckCircle v-else-if="Math.max(0, result.requiredFinalScore) <= parseFloat(form.hypotheticalFinal)" :size="16" class="text-green-500 mt-0.5 shrink-0" />
              ...
              <p class="text-xs font-medium text-gray-500 dark:text-gray-400 leading-relaxed">
                ...
                <span v-else-if="Math.max(0, result.requiredFinalScore) <= parseFloat(form.hypotheticalFinal)">Tuyệt vời! Điểm thi giả định {{ parseFloat(form.hypotheticalFinal).toFixed(1) }} vượt mức mong đợi, mục tiêu <b>KHẢ THI</b>!</span>
                <span v-else>Cần cố gắng ôn tập để có thể làm được đề đạt tối thiểu <b>{{ Math.max(0, result.requiredFinalScore).toFixed(1) }}đ</b>.</span>
              </p>
            </div>
          </div>
