...
const fetchWarnings = async () => {
  ...
  try {
    ...
    students.value = res.data.map(item => ({
      ...
      classId: item.maLop,
      subject: item.tenMon,
      currentAvg: item.diemTBMon !== null ? item.diemTBMon : 
                  // If tbMon is null, we can do a simple average for process grades just to display something, but API already provides NULL if empty. We compute a process average:
                  ((item.diemMieng * 0.1) + (item.diem15p * 0.1) + (item.diemGiuaKy * 0.3)), // Just naive display text if null
