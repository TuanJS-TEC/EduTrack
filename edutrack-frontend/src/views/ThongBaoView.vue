...
const statCards = [
  { id: 1, title: 'Cảnh báo', count: 3, newCount: 1, icon: AlertCircle, color: 'text-red-500 bg-red-50 dark:text-red-400 dark:bg-red-500/10', typeMatcher: 'Alert' },
  { id: 2, title: 'Học phí', count: 3, newCount: 1, icon: CreditCard, color: 'text-emerald-500 bg-emerald-50 dark:text-emerald-400 dark:bg-emerald-500/10', typeMatcher: 'Payment' },
  { id: 3, title: 'Tuyển sinh', count: 3, newCount: 1, icon: UserPlus, color: 'text-blue-500 bg-blue-50 dark:text-blue-400 dark:bg-blue-500/10', typeMatcher: 'Enrollment' },
  { id: 4, title: 'Điểm số', count: 3, newCount: 1, icon: FileText, color: 'text-orange-500 bg-orange-50 dark:text-orange-400 dark:bg-orange-500/10', typeMatcher: 'Grade' },
  { id: 5, title: 'Lịch trình', count: 2, newCount: 1, icon: Calendar, color: 'text-purple-500 bg-purple-50 dark:text-purple-400 dark:bg-purple-500/10', typeMatcher: 'Schedule' },
  { id: 6, title: 'Hệ thống', count: 2, newCount: 0, icon: Settings, color: 'text-gray-500 bg-gray-50 dark:text-gray-400 dark:bg-gray-500/10', typeMatcher: 'System' },
]

const sidebarLinks = [
  { name: 'Tất cả', icon: Bell, active: true },
  { name: 'Chưa đọc', icon: CheckCheck, active: false },
  { name: 'Cảnh báo', icon: AlertCircle, active: false },
  { name: 'Thanh toán', icon: CreditCard, active: false },
  { name: 'Tuyển sinh', icon: UserPlus, active: false },
  { name: 'Học tập', icon: FileText, active: false },
  { name: 'Hệ thống', icon: Settings, active: false },
]
  ...
  <div class="space-y-6">
    ...
    <div class="flex justify-between items-end">
      ...
      <div class="flex items-center gap-3">
        ...
        <button class="flex items-center gap-2 px-4 py-2 bg-white dark:bg-[#111C44] border border-gray-200 dark:border-white/10 rounded-lg text-sm font-bold text-gray-600 dark:text-gray-300 hover:bg-gray-50 dark:hover:bg-white/5 shadow-sm transition-colors hidden sm:flex">
          <CheckCheck :size="16" />
          Đánh dấu đã đọc
        </button>
      </div>
    ...
    <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-6 gap-4">
      <div v-for="stat in statCards" :key="stat.id" class="bg-white dark:bg-[#111C44] rounded-xl p-4 shadow-sm border border-gray-100 dark:border-white/5 flex flex-col items-center justify-center gap-2 cursor-pointer hover:border-blue-200 dark:hover:border-blue-500/50 hover:shadow-md transition-all">
        ...
        <div class="bg-white dark:bg-[#111C44] rounded-2xl shadow-sm border border-gray-100/50 dark:border-white/5 p-2 text-sm font-bold text-[#2B3674] dark:text-white">
          <button 
            ...
            :class="[
              ...
              link.active ? 'bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-50 dark:hover:bg-white/5'
            ]"
          >
            <div class="flex items-center gap-3">
              <component :is="link.icon" :size="18" :class="link.active ? 'text-blue-500 dark:text-blue-400' : 'text-gray-400 dark:text-gray-500'" />
              <span>{{ link.name }}</span>
            </div>
          </button>
        </div>
            ...
            <div class="flex-1 min-w-0">
              <div class="flex flex-col sm:flex-row sm:justify-between sm:items-start mb-1 gap-2">
                <h4 class="text-[15px] font-bold text-[#2B3674] dark:text-white flex items-center gap-2 truncate">
                  {{ notif.TieuDe }}
                  <span class="w-2 h-2 rounded-full bg-blue-500 dark:bg-blue-400 flex-shrink-0"></span>
                </h4>
              ...
              <div class="flex flex-col sm:flex-row justify-between items-start gap-4 border-t border-gray-50 dark:border-white/5 pt-4">
                ...
                <div class="flex items-center gap-4 text-xs font-bold shrink-0 mt-2 sm:mt-0">
                  <button class="flex items-center gap-1 text-[#1E88E5] dark:text-blue-400 hover:text-blue-600 dark:hover:text-blue-300 transition-colors">
                    <CheckCheck :size="14" />
                    Đã đọc
                  </button>
                  <button class="text-red-500 dark:text-red-400 hover:text-red-600 dark:hover:text-red-300 transition-colors flex items-center gap-1">
                    <Trash2 :size="14" />
                    Bỏ qua
                  </button>
                </div>
